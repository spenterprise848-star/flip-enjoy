    ...
    switch (payType) {
      case 'gpay':
        ...
        case 'phonepe':
            if (os === "android") {
                let payload = {
                    p2pPaymentCheckoutParams: {
                        ...
                        initialAmount: Math.round(amt * 100),
                        note: {
                            type: "text",
            ...
            if (os === "ios") {
                redirect_url = `phonepe://upi//pay?pa=${upi_address}&pn=${encodeURIComponent(site_name)}&am=${amt}&cu=INR&tn=${encodeURIComponent(orderNumber)}`;
            }
                //redirect_url = "phonepe://pay?ver=01&mode=19&pa=" + upi_address + "&pn=" + site_name + "&tr=&cu=INR&mc=7446&qrMedium=04&tn=" + site_name + "&am=" + amt;
